    ...
    try:
        ...
        if not filepath.endswith(".wav"):
            ...
            if low_pulse is not None:
                filter_parts.append(f"highpass=f={low_pulse}")
            if high_pulse is not None:
                filter_parts.append(f"lowpass=f={high_pulse}")
            if noise_reduction is not None:
                filter_parts.append(f"afftdn=nf={noise_reduction}")
            # Combine filter parts if any
            filter_string = ",".join(filter_parts) if filter_parts else None           
            output = ffmpeg.input(filepath)
            if filter_string:
                output = output.output(wav_filepath, ar=16000, ac=1, af=filter_string)
            else:
                output = output.output(wav_filepath, ar=16000, ac=1)
...
def transcribe(hp_dp,lp_dp,nr_dp,queue,File, Model, Computing, Lang, BeamSize, VadFilter, device):
    ...
    if hp_dp=="None":
        ...
        hp_dp=12000  
    queue.put(("print",f"高周波カット:{hp_dp}Hz"))
    ...
    if lp_dp=="None":
        ...
        lp_dp=150
    queue.put(("print",f"低周波カット:{lp_dp}Hz"))
    ...
    if nr_dp=="None":
        ...
        nr_dp=-20
    queue.put(("print",f"ノイズリダクション：{nr_dp}dB"))
...
def run_with_progress(hp_dp,lp_dp,nr_dp,File, Model, Computing, Lang, BeamSize, VadFilter, device,progress=gr.Progress()):
    ...
    queue = Queue()
    process = Process(target=transcribe, args=(hp_dp,lp_dp,nr_dp,queue,File, Model, Computing, Lang, BeamSize, VadFilter, device))
    process.start()
